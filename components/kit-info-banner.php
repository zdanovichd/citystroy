<div class="kit-info-banner-con">
	<div class="kit-info-banner">
			<!-- <img  class="kit-info-close__banner" src="../assets/img/icon/banner-close.svg" alt="" loading="lazy"> -->
			<img  class="kit-info__banner-img" src="../assets/img/icon/kit.svg" alt="" loading="lazy">
			<div>
				<p class="h1">БЕСПЛАТНО </p>
				<p class="h3">Концепция кухни для ресторана от 300м²</p>
				<p class="h2">Разрабатываем предварительные примерные планы зонирования, расстановки кухонного оборудования с размерами, планировки посадочных мест</p>
				<a href="/технология#tehno-plan-con"><p class="h2">Подробнее...</p></a>
			


			<form class="kit-form" action="#" method="POST" enctype="multipart/form-data">
						<input class="modal__input kit-input__name" type="text" data-validate-field="name" name="Имя" placeholder="Введите имя">
						<input class="modal__input kit-input__tel " type="tel" data-validate-field="tel" name="Телефон" placeholder="Введите телефон">
						<div class="modal__check-con kit-modal__check-con">
							<input id="kit__check-btn" type="checkbox">
							<label type="checkbox" class="modal__check-text" for="kit__check-btn">
								<span class="kit__check-btn modal__check"></span>
								Согласие на обработку данных
								<a href="../pages/assent.php"><img class="modal__check-que" src="/assets/icon/que.svg" alt="" loading="lazy"></a>
							</label>
					</div>
					<button  id="btn-kit-banner" class="button kit-button-banner">Отправить</button>
			</form>
			</div>
	</div>
</div>

